...
for i in range(len(all_data)):    
    ...
    if len(analysis_df)>1:
        avg_lap_time_reg = pd.to_datetime(analysis_df['time'][1:], format="%M:%S.%f").mean()
    else:
        avg_lap_time_reg = pd.to_datetime(analysis_df['time'], format="%M:%S.%f").mean()
    ...
    plt.savefig(f'/home/boris/Documents/matplotlib_exercize/LE_MANS_2024_MOTO_2/LE_MANS_2024_Analysis/{analysis_df.loc[0, "driver_name"]}_analysis.jpg')
    plt.close()