...
drivers = ['Collin VEIJER', 'David MUÑOZ', 'Ivan ORTOLA', 'Ryusei YAMANAKA', 'Joel KELSO',
           ...
           'Joel ESTEBAN', 'Hamad AL-SAHOUTI', 'Jose Antonio RUEDA', 'Jose Antonio RUED']
...
for i in range(len(all_data)):    
    ...
    if not analysis_df.empty:
        if len(analysis_df)>1:
            avg_lap_time_reg = pd.to_datetime(analysis_df['sector_1'][1:], format="%S.%f").mean()
        else:
            avg_lap_time_reg = pd.to_datetime(analysis_df['sector_1'], format="%S.%f").mean()
        ...
        plt.savefig(f'/home/boris/Documents/matplotlib_exercize/LE_MANS_2024_MOTO_3/LE_MANS_2024_Sector_1/{analysis_df.loc[0, "driver_name"]}_sector_1.jpg')
        plt.close()